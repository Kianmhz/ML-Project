...
for combo in combos:
    ...
    for tr_idx, vl_idx in kf.split(X_train_full, y_train_full):
        ...
        fold_f2 = max(
            fbeta_score(y_vl, (proba >= t).astype(int), beta=2, zero_division=0)
            for t in thresholds
        )
...
best_val_f2    = -1.0
for t in thresholds:
    f = fbeta_score(y_val2, (y_val_proba >= t).astype(int), beta=2, zero_division=0)
    if f > best_val_f2:
...
evaluate_and_print(y_test, y_pred_proba, best_threshold,   "XGBoost (max F2)")